...
class Trainer:
    ...
    def evaluate(
        ...
        # show detail information for auroc
        import pandas as pd
        predictions = torch.nn.functional.softmax(torch.from_numpy(output.predictions[0]), dim=-1).numpy()
        ...
        # detailInformation = dict()
        # for id, key in enumerate(eval_dataset['uid']):
        #     detailInformation[key] = list(output.predictions[id])

        # import csv
        # datas = []
        # header = list(detailInformation.keys())
        # datas.append(detailInformation)
        # with open(os.path.join(self.args.output_dir, 'detailInformation.csv'), 'w', newline='', encoding='utf-8') as f:
        #     writer = csv.DictWriter(f,fieldnames=header)
        #     writer.writeheader()
        #     writer.writerows(datas)
        # self.log(detailInformation)
