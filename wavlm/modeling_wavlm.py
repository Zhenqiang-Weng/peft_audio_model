...
class WavLMAttention(nn.Module):
    ...
    def torch_multi_head_self_attention(
        ...
        if hasattr(self.q_proj, 'base_layer'):
            q_proj_bias = self.q_proj.base_layer.bias
        else:
            q_proj_bias = self.q_proj.bias

        if hasattr(self.k_proj, 'base_layer'):
            k_proj_bias = self.k_proj.base_layer.bias
        else:
            k_proj_bias = self.k_proj.bias

        if hasattr(self.v_proj, 'base_layer'):
            v_proj_bias = self.v_proj.base_layer.bias
        else:
...
class WavLMForSequenceClassification(WavLMPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = loss_fct(logits.view(-1, self.config.num_labels), labels.view(-1).long())
