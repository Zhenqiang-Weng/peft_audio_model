...
class WhisperAdapter(nn.Module):
    def __init__(self, config):
        ...
        if config.d_model != config.hidden_size:
            self.proj = nn.Linear(config.d_model, config.output_hidden_size)
            self.proj_layer_norm = nn.LayerNorm(config.output_hidden_size)
        else:
            self.proj = self.proj_layer_norm = None

        self.layers = nn.ModuleList(WhisperAdapterLayer(config) for _ in range(config.num_adapter_layers))
